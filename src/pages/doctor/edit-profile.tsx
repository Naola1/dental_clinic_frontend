...
import { useUpdateProfileDoctor } from "@/hooks/use-auth";
import { usePatientProfile } from "@/hooks/use-profile";
import { DocUpdateProfileSchema } from "@/schema/auth";
...
const EditProfileDoctor = () => {
  ...
  const form = useForm<z.infer<typeof DocUpdateProfileSchema>>({
    resolver: zodResolver(DocUpdateProfileSchema),
    defaultValues: {
      user: {
  ...
  async function onSubmit(data: z.infer<typeof DocUpdateProfileSchema>) {
    setError(undefined);
